"""LISA AstroWG - MBH Catalogs - Catalog generation script - v2.0

Catalog v2.0 is primarily meant to collect 
...
There are some additional data, such as dark matter halo information, that can be optionally provided for v2.0 if desired.  

Links
-----
- Catalog v2.0 spreadsheet:
  https://docs.google.com/spreadsheets/d/1Zz-A5QcKHdPabpSmkKcs9ERzeAA0sqM_/edit#gid=1167697008
- MBHCatalogs github for code:
  https://github.com/mbonetti90/MBHCatalogs
- MBHCatalogs google drive for simulation data:
  https://drive.google.com/drive/folders/1XOaSj6xVCbuTXvVHxWT0oMeocWU_h_by?usp=share_link
...
ALL_DIR = "/hildafs/datasets/Asterix/subfind/reduced_catalog/"
...
def MBHB_no_delay(metadata):
    ...
    mdata = np.load(M_DIR + "lisa_mcat_v2.0_z1.3.npy")
    ...
    metadata['CommentsNoDelay'] = (
        "Simulation still running, z<1.3 data may be available later")
...
def MBHB_delay(metadata):
    ...
    # Required fields
        ...
        sepa_delay: separation at merger [pc]
        W: number density [Mpc^-3]
...
def MBH_population(metadata):
    ...
    for tarz in TARGET_REDSHIFTS:
        if tarz == 5:
            data = np.load(ALL_DIR + "ASTRID_subfind_galaxy_catalog_107.npy")
        if tarz == 2:
            data = np.load(ALL_DIR + "ASTRID_subfind_galaxy_catalog_348.npy")
        else:
            continue
...
#                "DarkMatterMass": mdm,
    ...
    metadata['CommentsAllBHs'] = (
        "No DM halo information available for now, will try to add it in later versions")
...
DATA_BINARIES_OPTIONAL = ["RemnantDarkMatterMass"]
        ...
        for k1 in data.keys():
            for k2 in data[k1].keys():
                for k3 in data[k1][k2].keys():
                    ...
                    ax.hist(data[k1][k2][k3],bins=50,range=(np.min(data[k1][k2][k3]),np.max(data[k1][k2][k3])),histtype='step')
                    ax.set_xlabel(k3)
    ...
    if choice == "NoDelay":
        ...
        for k1 in data.keys():
            for k2 in data[k1].keys():
                    ...
                    fig, ax=plt.subplots()
                    ax.set_title(f"{choice}: {k1}")
                    ax.hist(data[k1][k2],bins=50,range=(np.min(data[k1][k2]),np.max(data[k1][k2])),histtype='step')
                    ax.set_xlabel(k2)
...
def validate(h5in):
    ...
    def check_units_bounds(key, vals, check):
        ...
        if not good_lo or not good_hi:
            print("\n")
            warn(f"{key} values are nan or outside of expectations [{lo:.2e}, {hi:.2e}] units: {unit}. If value is nan please ensure this was intentional!")
    ...
    def check_expected_exist(namespace, specs, name, optional=[]):
        ...
        for key, check_vals in specs.items():
            ...
            if exists:
                check_units_bounds(key, namespace[key], check_vals)
            elif required:

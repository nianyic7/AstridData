...
def get_all_mfile_numbers(mroot):
    flist = sorted(glob.glob(mroot + "bh-merger-*.npy"))
    mlist = np.array([int(ff.split(".")[0].split("R")[-1]) for ff in flist])
    print("Loaded all merger files:", mlist, flush=True)
    return mlist


def find_mfile_this_snap(sthis, mlist):
    ...
    mask_pre = (mlist >= sthis) & (mlist < snext)
    mask_post = (mlist < sthis) & (mlist >= sprev)

    return mlist[mask_pre], mlist[mask_post], zprev, znext


def load_mergers(mlist, zmax, zmin):
    print("Loading mergers...")
    merger_list = []
    for n in mlist:
        ff = mroot + "bh-merger-R%03d.npy" % n
        merger = np.load(ff)
        mask = merger["z"] >= zmin
        mask &= merger["z"] < zmax
        merger_list.append(merger[mask])
    return merger_list
...
def get_snaps_with_stars(roots):
    ...
    for root in roots:
        ...
        for ff in ffs:
            if os.path.exists(ff + "/4/Position") and os.path.exists(
                ff + "/5/Position"
            ):
                pig = BigFile(ff)
                battr = pig["Header"].attrs
                scale_fac = pig["Header"].attrs["Time"][0]
                redshift = 1.0 / scale_fac - 1
...
def fit_density_plaw(z, r, rho, count, soft=1.5, scale=1.2):
    ...
    pars, cov = curve_fit(
        f=power_law, xdata=rr, ydata=pr, p0=[0, 0], bounds=(-np.inf, np.inf)
    )
...
def galaxy_info(bidx, profile=False):
    ...
    dr_p = dist_to_bh(pos=pos4, bpos=bpos, lbox=Lbox, scale=1.0 / (1 + redshift))
    mask = dr_p < rcut
    ...
    dv = 4.0 / 3 * np.pi * np.diff(bins**3) * 1e9
    ...
    prof_fit = fit_density_plaw(
        redshift, rr, gf1d(density, 2), count, scale=extrapolate_scale
    )
...
def get_galaxy_info_post(merger):
    ...
    try:
        bidx1 = id2idx[idr]
        mseed1, zseed1, m4tot1, mhtot1, sigma1, rho1, gamma1 = galaxy_info(
            bidx1, profile=True
        )
    except KeyError:
        mseed1, zseed1, m4tot1, mhtot1, sigma1, rho1, gamma1 = (
            -1,
            -1,
            -1,
            -1,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="path of the raw merger file directory",
    )
    parser.add_argument("--snap", required=True, type=int, help="pig snapshot number")
    parser.add_argument(
        "--rcut", required=True, type=float, help="radial cut for galaxy info, in kpc"
    )
    parser.add_argument(
        "--outdir", required=True, type=str, help="path of the output file directory"
    )
    ...
    mlist = get_all_mfile_numbers(mroot)
    mpre, mpost, zprev, znext = find_mfile_this_snap(sthis=snap, mlist=mlist)

    # ------------------ Pre-merger ------------------------------
    # mergers happening after the current snapshot (i.e. this snapshot is "pre" merger)
    # need znext to cutoff the boundary caes

    mergers_pre = load_mergers(mpre, zmax=redshift, zmin=znext)
    print("Loaded total number of pre-merger files:", len(mergers_pre), flush=True)
    ...
    for i, n in enumerate(mpre):
        savedir = outdir + "before/before-merger-gal-R%03d-S%03d.npy" % (n, snap)
        if os.path.isfile(savedir):
            print("skipping as already processed in %s" % savedir, flush=True)
            continue

        mergers = mergers_pre[i]
        print("getting pre-merger info for R-%03d..." % n, flush=True)
        info_pre = [get_galaxy_info_pre(m) for m in mergers]
    ...
    mergers_post = load_mergers(mpost, zmax=zprev, zmin=redshift)
    print("Loaded total number of post-merger files:", len(mergers_post), flush=True)
    ...
    # --------------- fetch galaxy info ------------------
    for i, n in enumerate(mpost):
        savedir = outdir + "after/after-merger-gal-R%03d-S%03d.npy" % (n, snap)
        if os.path.isfile(savedir):
            print("skipping as already processed in %s" % savedir, flush=True)
            continue

        mergers = mergers_post[i]
        print("getting post-merger info for R-%03d..." % n, flush=True)
        info_post = [get_galaxy_info_post(m) for m in mergers]
